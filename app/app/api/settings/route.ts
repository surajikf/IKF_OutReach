...
export async function GET(request: Request) {
    try {
        ...
        if (message.includes("Failed to decrypt")) {
            return error("INTERNAL_ERROR", "ENCRYPTION_MISMATCH: Existing encrypted settings cannot be decrypted with current STRATEGIC_SECRET. Run: npm run settings:repair-encryption", { status: 500 });
        }
